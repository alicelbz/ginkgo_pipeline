...
from FieldmapCorrection import runFieldmapCorrection
        ...
        def runPipeline(inputNiftiRoot, subjectJsonFileName, taskJsonFileName, session, outputDirectory, verbose):
            ...
            for subject, ses in _subjects_from_json(subjects):
                ...
                dir01_dc      = makeDirectory(subj_out, "01-DistortionCorrection")                 # NIfTI
                dir02_eddy    = makeDirectory(subj_out, "02-Eddy")                                 # NIfTI
                dir03_outlier = makeDirectory(subj_out, "03-OutlierCorrection")                    # NIfTI
                dir04_split   = makeDirectory(subj_out, "04-SplitDWIShells")                       # NIfTI (per-shell)
                dir05_gis     = makeDirectory(subj_out, "05-GisConversion")                        # GIS (per-shell + T1)
                dir06_orient  = makeDirectory(subj_out, "06-OrientationAndBValueFileDecoding")     # bridge
                dir07_qs      = makeDirectory(subj_out, "07-QSpaceSamplingAddition")               # NIfTI
                dir08_morph   = makeDirectory(subj_out, "08-MaskFromMorphologistPipeline")         # GIS-derived masks
                ...
                did_dc = False
                if method == "topup":
                    # Your AP/PA implementation writes dwi_topup.nii.gz → we standardize name to dwi_dc.nii.gz
                    runTopUpCorrection(session_dir, dir01_dc, verbose)
                    # standardize file name if needed
                    ...
                    # Your script should write dwi_dc.nii.gz in dir01_dc
                    ...
                    # Pas de correction → on propage le 4D tel quel
